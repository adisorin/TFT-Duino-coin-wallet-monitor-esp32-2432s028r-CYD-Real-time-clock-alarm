...
// ==== Duino-Coin ====
const char* ducoUser = "********";// user cange*******
...
void setup() {
  Serial.begin(115200);
  delay(50);
  if (EEPROM.read(ADDR_FLAG) == 1234) {
    EEPROM.get(ADDR_ALARM_VOL, alarmVolume);
    if (alarmVolume < 10 || alarmVolume > 255) alarmVolume = 180;
    }

  // Config PWM timer (folosit și pentru backlight)
  ledc_timer_config_t ledc_timer = {.speed_mode=LEDC_MODE,.duty_resolution=LEDC_RES,.timer_num=LEDC_TIMER,.freq_hz=LEDC_FREQ,.clk_cfg=LEDC_AUTO_CLK};
  ...
  ledc_channel_config_t ledc_channel = {.gpio_num=PIN_BACKLIGHT,.speed_mode=LEDC_MODE,.channel=LEDC_CHANNEL_BACKLIGHT,.intr_type=LEDC_INTR_DISABLE,.timer_sel=LEDC_TIMER,.duty=INITIAL_BACKLIGHT,.hpoint=0};
...
void loop() {
  ...
  if (currentMillis - lastClockUpdate > 1000) {
    if (paginaCurenta == 1) afiseazaCeas();
    desenButon();
    desenButonPagina();
    desenButonWifi();
